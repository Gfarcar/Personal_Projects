...
function AuthProvider({ children }) {
      ...
      try {
        ...
        if (accessToken && isValidToken(accessToken)) {
          ...
          const response = await axios.post(
            `${appsettings.apiUrl}/users/my-account`,
  ...
  const signin = async (email, password) => {
    const response = await axios.post(`${appsettings.apiUrl}/users/login`, {   //Cambiar
      email,
      password,
    }).catch(error => {
      throw error;
  ...
  const signup = async (email, password, nombre) => {
    const response = await axios.post(`${appsettings.apiUrl}/users/signup`, { //Cambair
      email,
      password,
      username: nombre,
    });
